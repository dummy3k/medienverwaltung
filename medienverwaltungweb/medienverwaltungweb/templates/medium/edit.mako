...
<p>
    ...
    <img src="${h.url_for(controller='image', action='thumbnail', id=c.item.id, width=200, height=300)}" class="plain"/>
    </a>
</p>
</div>
% endif
...
<%def name="side()">
    ...
    <h2>${_('Image')}:</h2>
        ...
        % endif
        <li><a href="${h.url_for(controller='image', action='upload', id=c.item.id)}">${_("Upload Image")}</a></li>
	</div>
