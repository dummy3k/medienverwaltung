...
<img class="plain" src="${h.url_for(controller='image', action='thumbnail', id=item.id, width=100, height=140, type=None, tag=None, page=None)}" />
</a>
% endfor
</p>
<p>${c.page.pager(controller='medium', action='list_gallery')}</p>
...
<%def name="side()">
	...
	<div class="box">
        ...
        <form method="post" action="${h.url_for(controller='medium', action='set_view_options', page=None)}">
